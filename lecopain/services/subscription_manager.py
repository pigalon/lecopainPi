...
class SubscriptionManager(): 
    ...
    def create_subscription(self, subscription, tmp_products, tmp_quantities, tmp_prices): 
        ...
        subscription = self.create_product_purchases(subscription, tmp_products, tmp_quantities, tmp_prices)
    ...
    def get_maps_from_subscriptions(self, subscriptions) : 
        ...
        for subscription in subscriptions : 
            ...
            customerMap[subscription.id] = str(customer.firstname + " " + customer.lastname)
 
            bought_items = Subscription_product.query.filter(Subscription_product.order_id == subscription.id).all()
            total = 0
            quantity = 0
