...
def subscriptions(customer_id):
    ...
    if customer_id == 0 or  customer_id == None:
        subscriptions = Subscription.query.order_by(Subscription.start.desc()).all()    
    else : 
        subscriptions = Subscription.query.filter(Subscription.customer_id==customer_id).order_by(Subscription.start.desc()).all()
...
def subscription_create():
    ...
    if form.validate_on_submit():
        subcription = Subscription(title=form.title.data, status=form.status.data, customer_id=int(form.customer_id.data), delivery_dt=form.delivery_dt.data)
        subscriptionServices.create_subscription(subcription=subcription, tmp_products=tmp_products, tmp_quantities=tmp_quantities, tmp_prices=tmp_prices)
